...
class Util
{
     ...
     * @return string
     */
    public static function recoverPublicKey(string $hash, string $r, string $s, int $v)
    {
        ...
        $secp256k1  = new EC('secp256k1');
        $publicKey  = $secp256k1->recoverPubKey($hash, [
            'r' => $r,
            's' => $s
        ], $v);
     ...
     */
    public static function ecsign(string $privateKey, string $message)
    {
        if (self::isHex($privateKey) === false) {
            throw new InvalidArgumentException('Invalid private key format.');
        }
