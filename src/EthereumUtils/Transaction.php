...
class Transaction
{
    ...
    public function sign(string $privateKey, int $chainId = 0): string {
        if ($chainId < 0) {
        ...
        $this->internalSign($privateKey, $chainId);

        return $this->RLPencode($this->getInput());
    }

    private function internalSign(string $privateKey, int $chainId): void {
